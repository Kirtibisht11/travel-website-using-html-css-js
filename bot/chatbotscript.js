...
const fetchCityTimeFromTimeZoneDB = async (city) => {
    ...
    for (const apiKey of apiKeys) {
        const url = `https://api.timezonedb.com/v2.1/get-time-zone?key=${apiKey}&by=city&city=${encodeURIComponent(city)}&format=json`;  // Use HTTPS
        ...
        try {
            ...
            if (data.status === "OK" && data.formatted) {
                ...
                appendMessage(`Sorry, I couldn't retrieve the time for ${city}. Response status: ${data.status}.`, "bot");
            }
        } catch (error) {
            console.error("Error fetching time:", error);
            appendMessage(`Error fetching time for ${city}. Please try again later.`, "bot");
        }
    }
...
const processMessage = async (message) => {
    ...
    if (lowerCaseMessage.includes("how are you")) {
        appendMessage("I'm doing great! How can I assist you today?", "bot");
        return;
    }

    if (lowerCaseMessage.includes("what can you do")) {
        appendMessage("I can tell you the current time and weather, and provide stock/finance-related definitions. How can I help you?", "bot");
        return;
    }

    if (lowerCaseMessage.includes("what is your name")) {
        appendMessage("My name is StockPal! I'm here to assist you with time, weather, and finance-related queries.", "bot");
        return;
    }

    // Handle "yes" or "no" responses
    if (lowerCaseMessage.includes("yes")) {
        appendMessage("Great! How can I assist you further?", "bot");
        return;
    }
    ...
    // Handle request for detailed or elaborate information
    const detailKeywords = ["detail", "elaborate", "summary"];
    if (detailKeywords.some(keyword => lowerCaseMessage.includes(keyword))) {
        const topic = message.replace(/detail|elaborate|summary/i, "").trim();
        if (topic) {
            const detailedInfo = await fetchDetailedInformation(topic);
            appendMessage(detailedInfo, "bot");
        } else {
            appendMessage("Please specify a topic you want more details about.", "bot");
        }
        return;
    }
    ...
    // Handle detailed queries (Wikipedia)
    appendMessage("Let me provide more information...", "bot");
    const detailedDefinition = await fetchDetailedDefinition(message.trim());
    appendMessage(detailedDefinition, "bot");
};

// Function to fetch detailed information from Wikipedia or predefined content
const fetchDetailedInformation = async (topic) => {
    if (stockDefinitions[topic.toLowerCase()]) {
        return stockDefinitions[topic.toLowerCase()];
    } else {
        return await fetchDetailedDefinition(topic);
    }
};

// Function to fetch detailed information from Wikipedia
const fetchDetailedDefinition = async (term) => {
    ...
    try {
        ...
        if (data.extract) {
            return `Here's a detailed explanation: ${data.extract}`;
        } else {
            return `I couldn't find detailed information for "${term}".`;
        }
    } catch (error) {
        console.error("Error fetching from Wikipedia:", error);
        return "Sorry, I couldn't fetch detailed information at the moment. Please try again later.";
    }
};
